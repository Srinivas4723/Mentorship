...
//				if(isMethodValid(newPlan.getMethods().get(i),oldPlanMethods) ) {
//						//&& isDateValid(newPlan.getFromdate(),newPlan.getTodate())) { -- date
...
//	@PostMapping("/createplan1/user/{userid}")
//	@PreAuthorize("hasRole('COMPENSATION')")
//	public ResponseEntity<?> addPlan(@Valid @RequestBody Plan plan,@PathVariable Long userid){
...
//		}
//		else if(existingPlan.isPresent()){
//			
//			//List<Method> mList= methodRepository.findByPlanid(existingPlan.get().getPlanid());
//			List<Method> mList=existingPlan.get().getMethods();
//			List<Integer> failedIndexes=new ArrayList<>();
//			for(int i=0;i<plan.getMethods().size();i++) {
//				if(!isMethodValid(plan.getMethods().get(i),mList)) {
//					failedIndexes.add(i);
//				}
//			}
//			if(failedIndexes.isEmpty()) {
//				existingPlan.get().getMethods().addAll(plan.getMethods());
//				//existingUser.get().getPlans().add(plan);
//				//userRepository.save(existingUser.get());
//				planRepository.save(existingPlan.get());
//				return ResponseEntity.ok().body(existingPlan.get().getPlanid().toString());
//			}
//			else {
//				return ResponseEntity.badRequest().body(mList);
//			}
//		}
//		else {
//			existingUser.get().getPlans().add(plan);
